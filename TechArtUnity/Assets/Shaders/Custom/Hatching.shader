    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_TARGET
            {
                ...
                half light = diffuse * mainLight.shadowAttenuation;
                light = step(_ShadingThreshold, light); // APPLY CEL SHADING.

                half4 shadow = SAMPLE_TEXTURE2D(_ShadowMap, sampler_BaseMap, input.uv);
                half4 textureColor = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, input.uv) * _BaseColor; 
                half4 final = lerp(shadow, textureColor, light);
